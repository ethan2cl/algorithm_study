 ...
 * @param s 生成的括号字符串
 */
function _recursion (left: number, right: number, n: number, result:string[], s: string) {
  // 递归出口
  if (left === n && right === n) {
    result.push(s)
    return
  ...
  if (left < n) _recursion(left + 1, right, n, result, s + '(')
  // 添加右括号
  // 剪枝条件: 必须左括号的数量比右括号的数量多
  if (left > right) _recursion(left, right + 1, n, result, s + ')')
}
